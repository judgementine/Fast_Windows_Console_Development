...
namespace FWC
{
	...
	void mouse_Pos(int &x, int &y)
	{
		POINT p;
		GetCursorPos(&p);
		ScreenToClient(Internals::G_window, &p);

		if (p.x < 0) p.x = 0;
		if (p.y < 0) p.y = 0;
		if (p.x >= Internals::G_console_rect.right) p.x = Internals::G_console_rect.right-1;
		if (p.y >= Internals::G_console_rect.bottom) p.y = Internals::G_console_rect.bottom-1;
		//is this ok?
		x = MulDiv(p.x, Internals::G_width, Internals::G_console_rect.right);
		y = MulDiv(p.y, Internals::G_height, Internals::G_console_rect.bottom);		
	}
	int key_State(unsigned char key)
	{
		return Internals::G_key_state[key];
	}
	int key_Prev_State(unsigned char key)
	{
		return Internals::G_prev_key_state[key];
	}
